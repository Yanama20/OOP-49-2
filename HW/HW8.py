...
# add_grade(3,'Алгебра', 3)
# add_grade(5, 'История', 2)
...
def create_grade_statistics_view():
    cursor.execute('''
        CREATE VIEW IF NOT EXISTS statistic_view AS
        SELECT MAX(grade) as max_grade, AVG(grade) as avg_grade, MIN(grade) as min_grade
        FROM grades
    ''')

def delete_view():
    cursor.execute('DROP VIEW IF EXISTS statistic_view')
    connect.commit()
